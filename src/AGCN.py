...
class AGCN(MessagePassing):
    ...
    def __init__(self, in_channels, out_channels, improved=False, cached=False,
        ...
        self.out_linear = torch.nn.Linear(in_channels, out_channels)
    ...
    def forward(self, x, edge_index, edge_weight=None, u=None):
        ...
        x0 = torch.matmul(torch.cat((x, u), dim=-1), self.weight[0])
        ...
        if not self.cached or self.cached_result is None:
            self.cached_num_edges = edge_index.size(1)
            edge_index1, norm = self.norm(edge_index, x.size(0), edge_weight[0],
        ...
        m0 = self.propagate(edge_index1, x=x0, norm=norm)
        
        x1 = torch.matmul(torch.cat((x, u), dim=-1), self.weight[1])
        ...
        if not self.cached or self.cached_result is None:
            self.cached_num_edges = edge_index.size(1)
            edge_index2, norm = self.norm(edge_index, x.size(0), edge_weight[1],
                                          self.improved, x.dtype)
        ...
        m1 = self.propagate(edge_index2, x=x1, norm=norm)
